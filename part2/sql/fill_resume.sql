INSERT INTO hh_import.resume (account_id, name, city, position, shedule, education_level, experience_years, salary,
                              about, status)
SELECT round((({1}-0.1)*random())+0.5),
       ...
       'about' || i,
       CASE WHEN (random() < 0.8) THEN hh.RESUME_STATUS('SHOW') ELSE hh.RESUME_STATUS('HIDE') END
FROM generate_series(1, {0}) AS g (i) RETURNING resume_id
